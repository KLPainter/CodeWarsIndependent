...
function findWholeShip(row, column, field) {
  const ship = [];
  isVertical = field[row + 1][column] === 1;
...
function isTouchingAnother(ship, field) {
  ...
  minRow = Math.max(ship[0][0] - 1, 0);
  maxRow = Math.min(ship[shipLength - 1][0] + 1, 9);
  minColumn = Math.max(ship[0][1] - 1, 0);
  maxColumn = Math.min(ship[shipLength - 1][1] + 1, 9);
